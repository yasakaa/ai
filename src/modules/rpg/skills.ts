...
export const skills: Skill[] = [
  ...
  },
  {
    ...
    info: `ステータス+(敵の攻撃 × 敵の防御 / 4)% (+10%以上は上昇率鈍化)`,
  ...
  },
  {
    ...
    short: '乱',
    desc: `乱数幅が20~180 -> 5~225になります クリティカル率も上がります`,
    info: `乱数幅 20~180 -> 5~225 (%) 期待値 115% クリティカル率+2% 乱数系と重複しない`,
    effect: { atkRndMin: -0.15, atkRndMax: 0.6, critUpFixed: 0.02 },
    unique: 'rnd',
  },
  {
...
export function aggregateSkillsEffects(data: any): SkillEffect {
  ...
  if (aggregatedEffect.rainbow && aggregatedEffect.rainbow > 1) {
    aggregatedEffect.atkUp =
      (aggregatedEffect.atkUp ?? 0) *
      (1 + (aggregatedEffect.rainbow - 1) * 0.05);
    aggregatedEffect.defUp =
      (aggregatedEffect.defUp ?? 0) *
      (1 + (aggregatedEffect.rainbow - 1) * 0.05);
  ...
  if (aggregatedEffect.distributed) {
    ...
    if (count < 3) {
      aggregatedEffect.atkUp =
        (aggregatedEffect.atkUp ?? 0) *
        (1 + aggregatedEffect.distributed * (1 - count * 0.4));
      aggregatedEffect.defUp =
        (aggregatedEffect.defUp ?? 0) *
        (1 + aggregatedEffect.distributed * (1 - count * 0.4));
  ...
  if (aggregatedEffect.abortDown && aggregatedEffect.abortDown > 1) {
    aggregatedEffect.atkUp =
      (aggregatedEffect.atkUp ?? 0) *
      (1 + (aggregatedEffect.abortDown - 1) * (1 / 3));
  ...
  if (aggregatedEffect.enemyCritDown && aggregatedEffect.enemyCritDown > 1) {
    aggregatedEffect.defUp =
      (aggregatedEffect.defUp ?? 0) *
      (1 + (aggregatedEffect.enemyCritDown - 1) * (1 / 3));
...
export function getTotalEffectString(data: any, skillX = 1): string {
  ...
  if (skillEffects.dart) {
    resultS.push(
      'ターン内最大ダメージ: +' + showNum(skillEffects.dart * 100) + '%',
    );
  ...
  if (skillEffects.abortDown) {
    resultS.push(
      '連続攻撃中断回避率: +' + showNum(skillEffects.abortDown * 100) + '%',
    );
  ...
  if (skillEffects.light) {
    resultS.push(
      '戦闘時被ダメージ半減率: ' + showNum(skillEffects.light * 100) + '%',
    );
  ...
  if (skillEffects.dark) {
    ...
    resultS.push(
      '戦闘時固定ダメージ付与率: ' +
        showNum((skillEffects.dark ?? 0) * 100) +
        '%',
    );
  ...
  if (skillEffects.notRandom || aggregateTokensEffects(data).notRandom) {
    result.push(
      '与ダメージ乱数固定: ' +
        showNum((atkMinRnd + atkMinRnd + atkMaxRnd) * 100) *
    ...
    if (atkMinRnd !== 0.2 || atkMaxRnd !== 1.6) {
      result.push(
        '与ダメージ乱数幅: ' +
          showNum(atkMinRnd * 100) +
          '% ～ ' +
          showNum((atkMinRnd + atkMaxRnd) * 100) +
          '%',
      );
    }
  }
  ...
  if (skillEffects.firstTurnResist) {
    if (skillEffects.firstTurnResist > 1) {
      ...
      result.push(
        'ターン2ダメージ軽減: ' +
          showNum((skillEffects.firstTurnResist - 1) * 100) +
          '%',
      );
    } else {
      result.push(
        'ターン1ダメージ軽減: ' +
          showNum(skillEffects.firstTurnResist * 100) +
          '%',
      );
    }
  }
  if (skillEffects.tenacious) {
    if (skillEffects.tenacious > 0.9) {
      ...
      result.push(
        '（体力' +
          showNum((1 - 0.9 / skillEffects.tenacious) * 100) +
          '%で効果最大）',
      );
    } else {
      result.push(
        'ピンチダメージ軽減: 最大' +
          showNum(skillEffects.tenacious * 100) +
          '%',
      );
    }
  }
  ...
  if (defMinRnd !== 0.2 || defMaxRnd !== 1.6) {
    result.push(
      '被ダメージ乱数幅: ' +
        showNum(defMinRnd * 100) +
        '% ～ ' +
        showNum((defMinRnd + defMaxRnd) * 100) +
        '%',
    );
  }
  ...
  if (skillEffects.critUp) {
    result.push(
      'クリティカル率（割合）: +' +
        showNum((skillEffects.critUp ?? 0) * 100) +
        '%',
    );
  }
  if (skillEffects.haisuiCritUp) {
    result.push(
      '覚悟クリティカル率（割合）: +' +
        showNum((skillEffects.haisuiCritUp ?? 0) * 100) +
        '%',
    );
  }
  if (skillEffects.critUpFixed) {
    result.push(
      'クリティカル率（固定）: +' +
        showNum((skillEffects.critUpFixed ?? 0) * 100) +
        '%',
    );
  }
  ...
  if (skillEffects.enemyCritDown) {
    result.push(
      '敵クリティカル率: -' +
        showNum((skillEffects.enemyCritDown ?? 0) * 100) +
        '%',
    );
  }
  if (skillEffects.enemyCritDmgDown) {
    result.push(
      '敵クリティカルダメージ: -' +
        showNum((skillEffects.enemyCritDmgDown ?? 0) * 100) +
        '%',
    );
  }
  if (skillEffects.haisuiUp) {
    result.push(
      '決死の覚悟効果量: +' + showNum((skillEffects.haisuiUp ?? 0) * 100) + '%',
    );
    result.push(
      '決死の覚悟発動体力: ' +
        showNum((1 / 7) * (1 + (skillEffects.haisuiUp ?? 0)) * 100) +
        '%以下',
    );
  }
  if (skillEffects.finalAttackUp) {
    result.push(
      '全力の一撃ダメージ: +' +
        showNum((skillEffects.finalAttackUp ?? 0) * 100) +
        '%',
    );
  }
  if (skillEffects.guardAtkUp) {
    result.push(
      `がまんパワーアップ${(skillEffects.guardAtkUp ?? 0) > 1 ? ` ×${showNum(skillEffects.guardAtkUp ?? 0)}` : ''}`,
    );
  }
  ...
  if (skillEffects.itemEquip) {
    result.push(
      'アイテム装備率: +' + showNum((skillEffects.itemEquip ?? 0) * 100) + '%',
    );
  }
  ...
  if (skillEffects.poisonAvoid) {
    result.push(
      '毒食べ物回避率: ' + showNum((skillEffects.poisonAvoid ?? 0) * 100) + '%',
    );
  }
  if (skillEffects.mindMinusAvoid) {
    result.push(
      '悪アイテム回避率: +' +
        showNum((skillEffects.mindMinusAvoid ?? 0) * 100) +
        '%',
    );
  }
  ...
  if (isSuper && !aggregateTokensEffects(data).redMode)
    itemResist = itemResist / 2;
  ...
  if (skillEffects.itemBoost) {
    result.push(
      'アイテム気合上昇率: +' +
        showNum((skillEffects.itemBoost ?? 0) * 100) +
        '%',
    );
  }
  ...
  if (
    ...
    result.push(
      `ランダムステータス${(skillEffects.fortuneEffect ?? 0) !== 1 ? ((skillEffects.fortuneEffect ?? 0) > 1 ? ` ×${showNum(skillEffects.fortuneEffect ?? 0)}` : `: ${showNum(skillEffects.fortuneEffect ?? 0)}`) : ''}`,
    );
  }
  ...
  if (boost) {
    result.push(
      'お守り耐久減少率: ' +
        showNum((1 / Math.pow(1.5, boost * 2)) * 100) +
        '%',
    );
  }
  if (skillEffects.priceOff) {
    result.push(
      'ショップ割引率: ' + showNum((skillEffects.priceOff ?? 0) * 100) + '%',
    );
  }

  const totalAtk =
    (1 + atk) *
    Math.max(1 + bAtk, 1 + nbAtk) *
    (1 + (skillEffects.haisuiAtkUp ?? 0)) *
    (1 + spd) *
    (1 + bSpd) *
    (1 / (1 + eDef)) *
    (1 + dmgBonus) *
    ((atkMinRnd + atkMinRnd + atkMaxRnd) *
      (0.5 + (skillEffects.notRandom ?? 0) * 0.05)) *
    (1 +
    ...
    (1 +
      Math.min(
        0.4 * (skillEffects.itemEquip ?? 0) +
          (skillEffects.firstTurnItem ? 1 / 6 : 0),
        1,
      ) *
        ((1 + (skillEffects.weaponSelect ?? 0)) /
          (4 +
  ...
  if (totalAtk > 1) {
    result.push('');
    result.push(
      '合計攻撃効果（最大）: +' + showNum((totalAtk - 1) * 100) + '%',
    );
  }

  const totalDef =
    (1 / (1 + def)) *
    (1 / Math.max(1 + bDef, 1 + nbDef)) *
    (1 + eAtk) *
    (1 + defDmgX) *
        ...
        Math.min(
          ...
          ((1 +
            ...
            (4 +
              ...
              (skillEffects.foodSelect ?? 0) -
              (skillEffects.poisonAvoid ?? 0))) *
          (0.25 * (1 + itemDef) + 0.25 * (1 + itemFood))));
