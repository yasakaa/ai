...
export default class extends Module {
  ...
  private handleRecordCommands(msg: Message) {
    ...
    if (msg.includes(['ランク'])) {
      ...
      message.push(
        createRankMessage(null, '最大木人ダメージ', 'bestScore', {
          suffix: 'ダメージ',
        }),
      );
      message.push(
        createRankMessage(null, '修行モード最高クリア記録', 'maxEndress', {
          prefix: 'ステージ',
          addValue: 1,
        }),
      );
      message.push(
        createRankMessage(null, '運の良さ', 'maxStatusUp', { suffix: 'pts' }),
      );
      message.push(
        createRankMessage(null, 'チェキ購入数', 'jar', { suffix: '枚' }),
      );
      if (data.raidScore) {
        for (const [key, value] of Object.entries(data.raidScore)) {
          if (value && typeof value === 'number') {
            ...
            message.push(
              `${createRankMessage(null, key + ` 最大${enemy?.scoreMsg ?? 'ダメージ'}`, `raidScore.${key}`, { suffix: data.clearRaid?.includes(key) ? `${enemy?.scoreMsg2 ?? 'ダメージ'} ⭐️` : `${enemy?.scoreMsg2 ?? 'ダメージ'}` })}`,
            );
          }
        }
      }
      ...
      if (data.jar) {
        message.push(
          createRankMessage(data.jar, 'チェキ購入数', 'jar', { suffix: '枚' }),
        );
      }
      ...
      if (data.raidScore) {
        for (const [key, value] of Object.entries(data.raidScore)) {
          if (value && typeof value === 'number') {
            const enemy = raidEnemys.find((x) => x.name === key);
            const score = enemy?.power
              ? Math.max(
                  Math.log2(
                    (value * 20) / (1024 / ((enemy.power ?? 30) / 30)),
                  ) + 1,
                  1,
                )
              : undefined;
  ...
  private handleAdminCommands(msg: Message) {
    ...
    if (msg.includes(['startRaid'])) {
      start(
        undefined,
        msg.includes(['recent']) ? 'r' : msg.includes(['hato']) ? 'h' : '',
      );
      return { reaction: 'love' };
    }
    ...
    message += [
      ...
      data.lv >= rpgData.maxLv
        ...
        : data.lastOnemorePlayedAt !== getDate()
          ? '(無料でおかわり可能)'
          : data.coin >= needCoin
            ? `(${config.rpgCoinShortName}でおかわり可能 ${data.coin > 99 ? '99+' : data.coin} / ${needCoin})`
            : '',
    ]
      .filter(Boolean)
      .join('\n');
  ...
  private async rpgAccountListAdd() {
    ...
    if (this.rpgPlayerList) {
      ...
      const rpgPlayers = friends.filter(
        (x) =>
          x.perModulesData?.rpg?.lv &&
          x.perModulesData?.rpg?.lv >= 20 &&
          !x.perModulesData?.rpg?.isBlocked,
      );
