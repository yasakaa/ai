...
export const shopItems: ShopItem[] = [
  ...
  },
  {
    ...
    price: 1,
    desc: `大晦日のレイド時、体力が1で開始しますが与えるダメージが25%増加します 自身の与えたダメージによって、追加でもこコインを得ます`,
    type: 'token',
  ...
  },
  {
    ...
    limit: (data) =>
      data.lv < 254 &&
      data.maxLv > 254 &&
      (data.info === 3 || data.coin >= lvBoostPrice(data) * 0.8),
    price: lvBoostPrice,
...
function getRandomSkills(ai, num) {
  ...
  let totalWeight = filteredSkills.reduce((total, skill) => {
    const skillCount =
      !skill.cantReroll && !skill.notLearn
...
export const shopReply = async (module: rpg, ai: 藍, msg: Message) => {
  ...
  if (data.lastShopVisited !== getDate() || !data.shopItems?.length) {
    ...
    data.shopItems = [
      ...
      data.lastBreakItem && Math.random() < 1
        ? data.lastBreakItem
        : getShopItems(),
