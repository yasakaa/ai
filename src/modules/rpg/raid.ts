...
export async function start(triggerUserId?: string, flg?: any) {
  ...
  if (flg?.includes('h'))
    enemy = raidEnemys.find((x) => x.name === ':hatoguruma:') ?? enemy;
...
function finish(raid: Raid) {
  ...
  if (!raid.attackers?.filter((x) => x.dmg > 1).length) {
    ...
    ai.post({
      text: raid.enemy.power
        ...
        : serifs.rpg.onagare2(raid.enemy.name),
  ...
  if (rpgData) {
    if (score && raid.enemy.power) {
      ...
      const sum1 =
        rpgData.raidReputations.length > 0
          ? rpgData.raidReputations.reduce((acc, cur) => acc + cur, 0)
          : 0;
      const reputation1 =
        rpgData.raidReputations.length > 0
          ? sum1 / rpgData.raidReputations.length
          : 0;
      ...
      bonusCoin = Math.min(
        Math.max(1, Math.floor(reputation2 * 16.75 * 1.5 ** reputation2) / 750),
        11,
      );

      if (reputation1 == 0) {
        results.push(
          `討伐隊の評判値: ${Math.floor(reputation2 * 16.75 * 1.5 ** reputation2).toLocaleString()} ↑アップ！`,
        );
      } else {
        results.push(
          `討伐隊の評判値: ${Math.floor(reputation1 * 16.75 * 1.5 ** reputation1).toLocaleString()} → ${Math.floor(reputation2 * 16.75 * 1.5 ** reputation2).toLocaleString()} ${reputation1 < reputation2 ? '↑アップ！' : reputation1 > reputation2 ? '↓ダウン…' : ''}`,
        );
      }

      if (score != Math.floor((score ?? 4) * bonusCoin)) {
        results.push(
          `評判値ボーナス！ キレイなどんぐり+${Math.floor((score ?? 4) * bonusCoin) - score}個`,
        );
      }
    }
    if (!rpgData.raidScore) rpgData.raidScore = {};
  ...
  if (sortAttackers.length >= 3) {
    const luckyUser =
      sortAttackers[
        scoreRaw ? Math.floor(Math.random() * sortAttackers.length) : 0
      ].user;
    const bonus = Math.ceil(
      (sortAttackers.length / 5) * (scoreRaw ?? sortAttackers[0].dmg / 10),
    );
    results.push(
      (scoreRaw ? '\nラッキー！: ' : '優勝！: ') +
        acct(luckyUser) +
  ...
  const text =
    ...
    '\n\n' +
    (score
      ? serifs.rpg.finish(raid.enemy.name, Math.floor((score ?? 4) * bonusCoin))
      : serifs.rpg.finish2(raid.enemy.name, 4));
  ...
  sortAttackers.forEach((x) => {
    ...
    data.raidAdjust =
      (data.raidAdjust ?? 0) + Math.round(winCount - loseCount * (1 / 3));
    friend.setPerModulesData(module_, data);
  });
  ...
  ai.post({
    text: text,
    cw: score
      ? serifs.rpg.finishCw(raid.enemy.name)
      : serifs.rpg.finishCw2(raid.enemy.name),
    renoteId: raid.postId,
...
export async function raidContextHook(key: any, msg: Message, data: any) {
  ...
  if (!_data.lv) {
    msg.reply(
      `レイドボスに参加しようとしてくれてありがとうなのじゃ！\nレイドボスに参加するには、通常のRPGモードを先に1回プレイする必要があるのじゃ！（わらわにRPGと話しかけてほしいのじゃ！）\nその後もう一度さっきの投稿に話しかけてくれれば、レイドボスに参加できるのじゃ！`,
    );
    return {
      reaction: 'hmm',
    };
...
export async function getTotalDmg(msg, enemy: RaidEnemy) {
  ...
  if (data.lv >= 20) {
    ...
    if (
      ...
      if (
        ...
        (data.skills?.length < 3 && Math.random() < 0.5)
      ) {
        wakabaFlg = true;
        data.items.push({
          name: `わかばのお守り`,
          price: 1,
          desc: `阨ちゃんの持っているスキルが5個より少ない場合（阨ちゃんのレベルが低い場合）、少ないスキル1つにつき約6%分パワー・防御が上がります 特定条件でさらにパワー・防御が+12%されます 耐久20`,
          type: 'amulet',
          effect: { beginner: 0.06 },
          durability: 20,
          short: '🔰',
        });
      } else {
  ...
  if (enemy.forcePostCount) {
    ...
    continuousBonusX =
      getRaidPostX(postCount + continuousBonusNum) / getRaidPostX(postCount);
  ...
  if (enemy.forcePostCount) {
    ...
    if (aggregateTokensEffects(data).showPostBonus) {
      ...
      if (verboseLog && continuousBonusX >= 1.01)
        message +=
          '連スキル効果: AD+' + Math.ceil(continuousBonusX * 100 - 100) + '%\n';
      if (verboseLog && tp - rawTp >= 0.01)
        message +=
          '投スキル効果: AD+' + Math.ceil((tp / rawTp) * 100 - 100) + '%\n';
    }
  } else {
    if (aggregateTokensEffects(data).showPostBonus) {
      buff += 1;
  ...
  const formatNumber = (num: number): string => {
    ...
    if (num < 10_000_000) {
      ...
      return result < 100
        ? (Math.round(result * 100) / 100).toString() + 'k'
        : result < 1000
          ? (Math.round(result * 10) / 10).toString() + 'k'
          : Math.round(result).toString() + 'k';
    } else {
      const result = num / 1_000_000;
      return result < 100
        ? (Math.round(result * 100) / 100).toString() + 'm'
        : result < 1000
          ? (Math.round(result * 10) / 10).toString() + 'm'
          : Math.round(result).toString() + 'm';
    }
  };
  ...
  let bonusX =
    (day === 6 || day === 0 || stockRandomResult.activate
      ? 1
      : (Math.floor(
          seedrandom(
            '' +
  ...
  const checkMagic = (phase, argTriggerData = {} as any) => {
    ...
    const useMagics = data.magic.filter(
      (m) =>
        m.phase === phase && (m.trigger(triggerData) || Math.random() < 0.08),
    );
  ...
  for (let actionX = 0; actionX < plusActionX + 1; actionX++) {
    ...
    const itemEquip =
      (0.4 + (1 - playerHpPercent) * 0.6 + itemBoost) * (slothFlg ? 0 : 1);
    ...
    if (
      ...
      if (enemy.pLToR) {
        ...
        const mEffect = Math.max(
          count !== 1 ? 0 : skillEffects.firstTurnItemChoice,
          itemMinEffect,
        );
        if (
        ...
        if (skillEffects.greed) {
          ...
          if (match && match.index !== undefined) {
            ...
            data.stockItem.name =
              data.stockItem.name.slice(0, match.index) + '-' + newNumber;
          } else {
        ...
        if (
          count === 1 &&
          skillEffects.firstTurnDoubleItem &&
          Math.random() < itemEquip * (1 + (skillEffects.itemEquip ?? 0))
        ) {
          if (
            (type === 'weapon' && !(isBattle && isPhysical)) ||
        ...
        case 'poison':
          ...
          if (enemy.pLToR) {
            ...
            const dmg = Math.round(
              playerHp *
                (item.effect * 0.003) *
                (isSuper && !aggregateTokensEffects(data).redMode ? 0.5 : 1),
            );
            playerHp -= dmg;
    ...
    if (skillEffects.allForOne || aggregateTokensEffects(data).allForOne) {
      ...
      if (itemBonus?.atk)
        itemBonus.atk =
          itemBonus.atk * spdx * (1 + (skillEffects.allForOne ?? 0) * 0.1);
      spd = 1;
    ...
    for (let i = 0; i < spd; i++) {
      ...
      let dmgBonus =
        Math.max(1 + (skillEffects.atkDmgUp ?? 0) * dmgUp, atkMinusMin) *
          turnDmgX +
      ...
      if (enemy && skillEffects.envy) {
        ...
        if (rate < 1) {
          ...
          const score = enemy
            ? Math.max(
                Math.log2(
                  (totalDmg * 20) / (1024 / ((enemy.power ?? 30) / 30)),
                ) + 1,
                1,
              )
            : undefined;
          ...
          (data.skills?.length >= 4
            ...
            : data.skills?.length >= 3
              ...
              : data.skills?.length >= 2
                ? 0.6
                : 0.9) *
            Math.max(1 - totalDmg / targetScore, 0);
        if (rate < 1) {
          enemyAtkX = enemyAtkX * rate;
          if (verboseLog) {
            buff += 1;
  ...
  if (verboseLog) {
    const score = enemy
      ? Math.max(
          Math.log2((totalDmg * 20) / (1024 / ((enemy.power ?? 30) / 30))) + 1,
          1,
        )
      : undefined;
    if (score) message += `\n評価: ★${score.toFixed(2)}`;
  ...
  if (
    data.exp >= 5 &&
    data.lv != 254 &&
    (data.lv > 255 || data.lv + 1 < rpgData.maxLv)
  ) {
...
export async function getTotalDmg2(msg, enemy: RaidEnemy) {
  ...
  for (let actionX = 0; actionX < plusActionX + 1; actionX++) {
    ...
    if (count === 1 || rnd) {
      ...
      let dmg = Math.round(
        Math.round(
          (500 * Math.max(attackCount, 1) * (1 + dmgup) * 1.5 ** drawCount) /
            250,
        ) * 250,
      );
      drawCount = 0;
  ...
  if (playerHp > 0) {
    ...
    let dmg = Math.round(
      Math.round(
        (500 * Math.max(attackCount, 1) * (1 + dmgup) * 1.5 ** drawCount) / 250,
      ) * 250,
    );
    if (attackCount >= 7) {
...
export async function getTotalDmg3(msg, enemy: RaidEnemy) {
  ...
  if (skillEffects.notBattleBonusDef > 0) {
    buff += 1;
    message +=
      `疲れにくい 器用さ+${Math.ceil(skillEffects.notBattleBonusDef * 25)}%` +
      `\n`;
  ...
  if (skillEffects.noAmuletAtkUp > 0) {
    buff += 1;
    message +=
      `かるわざ 器用さ+${Math.ceil(skillEffects.noAmuletAtkUp * 200)}%` + `\n`;
  ...
  if (skillEffects.plusActionX > 0) {
    buff += 1;
    message +=
      `高速RPG 器用さ+${Math.ceil(skillEffects.plusActionX * 8)}%` + `\n`;
  ...
  if (skillEffects.atkRndMin > 0) {
    buff += 1;
    message +=
      `安定感 器用さ+${Math.ceil(skillEffects.atkRndMin * 20)}%` + `\n`;
  ...
  if (skillEffects.itemBoost > 0) {
    buff += 1;
    message +=
      `道具効果量 器用さ+${Math.ceil(skillEffects.itemBoost * (100 / 5))}%` +
      `\n`;
  ...
  if (skillEffects.mindMinusAvoid > 0) {
    buff += 1;
    message +=
      `道具の選択が上手い 器用さ+${Math.ceil(skillEffects.mindMinusAvoid * (100 / 3))}%` +
      `\n`;
  ...
  if (skillEffects.amuletPower > 1) {
    buff += 1;
    message +=
      `お守りパワー 器用さ+${Math.ceil((skillEffects.amuletPower - 1) * 3)}%` +
      `\n`;
  ...
  if (skillEffects.abortDown > 0) {
    buff += 1;
    message +=
      `連続攻撃完遂率上昇 仕上げ+${Math.ceil(skillEffects.abortDown * 25)}%` +
      `\n`;
  ...
  if (skillEffects.tenacious > 0) {
    buff += 1;
    message +=
      `粘り強さ 仕上げ+${Math.ceil(skillEffects.tenacious * 25)}%` + `\n`;
  ...
  if (skillEffects.endureUp > 0) {
    buff += 1;
    message +=
      `気合で頑張る 仕上げ+${Math.ceil(skillEffects.endureUp * 15)}%` + `\n`;
  ...
  if (!data.raidScore[enemy.name] || data.raidScore[enemy.name] < totalDmg) {
    if (
      data.raidScore[enemy.name] &&
      Math.floor(data.raidScore[enemy.name]) != Math.floor(totalDmg)
    ) {
      message += '過去最高の手応えだ！' + `\n\n`;
      if (mark === ':blank:') mark = '🆙';
    }
