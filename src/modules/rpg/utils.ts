...
export function initializeData(module: rpg, msg) {
  ...
  if (
    Array.isArray(data.bankItems) &&
    data.bankItems.some((x) => x === '選択カードの札')
  ) {
    data.bankItems = data.bankItems.map((name) =>
      name === '選択カードの札' ? '質問カードの札' : name,
    );
  }
  if (
    Array.isArray(data.shopItems) &&
    data.shopItems.some(
      (name) => !Array.isArray(name) && name === '選択カードの札',
    )
  ) {
    data.shopItems = data.shopItems.map((name) =>
      Array.isArray(name)
        ? name
        : name === '選択カードの札'
          ? '質問カードの札'
  ...
  if (
    Array.isArray(data.shop2Items) &&
    data.shop2Items.some(
      (name) => !Array.isArray(name) && name === '選択カードの札',
    )
  ) {
    data.shop2Items = data.shop2Items.map((name) =>
      Array.isArray(name)
        ? name
        : name === '選択カードの札'
          ? '質問カードの札'
  ...
  if (
    Array.isArray(data.showShopItems) &&
    data.showShopItems.some((item) => item.name === '選択カードの札')
  ) {
    data.showShopItems = data.showShopItems.map((item) => ({
      ...item,
      name: item.name === '選択カードの札' ? '質問カードの札' : item.name,
    }));
  }
...
export async function getPostCount(
  ...
  const usageField = usage
    ? usage.type === 'raid'
      ? 'lastRaidPostCountUsageKey'
      : 'lastPostCountUsageKey'
    : null;
      ...
      for (const userId of msg.friend.doc.linkedAccounts) {
        ...
        if (linkData.noChart && linkData.todayNotesCount) {
          linkedPostCount = Math.max(
            (friend.doc.user?.notesCount ?? linkData.todayNotesCount) -
              linkData.todayNotesCount,
            linkData.todayNotesCount -
              (linkData.yesterdayNotesCount ?? linkData.todayNotesCount),
          );
        } else {
    ...
    if (msg.friend.doc.linkedAccounts?.length) {
      for (const userId of msg.friend.doc.linkedAccounts) {
        ...
        if (friendChart?.diffs) {
          linkedPostCount = Math.max(
            (friendChart.diffs.normal?.[0] ?? 0) +
              (friendChart.diffs.reply?.[0] ?? 0) +
              (friendChart.diffs.withFile?.[0] ?? 0),
            (friendChart.diffs.normal?.[1] ?? 0) +
              (friendChart.diffs.reply?.[1] ?? 0) +
              (friendChart.diffs.withFile?.[1] ?? 0),
          );
        }
...
export function preLevelUpProcess(data): string {
  ...
  if (
    (data.info ?? 0) < 1 &&
    100 + data.lv * 3 + (data.winCount ?? 0) * 5 >= 300
  ) {
    data.info = 1;
    addMessage += `\n` + serifs.rpg.info;
  }
